...
class KMeans:
    ...
    def train(self, x_train):
        ...
        cluster_centers_  = x_train[centroid_index, :]
        ...
        while np.linalg.norm(cluster_centers_ - last_cluster_centers_) >= self.tol \
            ...
            for i in range(num_of_data):
                S[i] = np.argmin(np.linalg.norm(x_train[i, :] - cluster_centers_, axis=1))
            #***********Update******************
            for i in range(self.n_clusters):
                index_list = np.where(S==i)[0]
                if not len(index_list) == 0:                    
                    cluster_centers_[i, :] = np.mean(x_train[index_list, :], axis=0)
            iteration_cnt += 1
